...
def save_sales_department(name, address):
    new_dept = {"name": name, "address": address}
    save_to_file(new_dept, "database/sales.json")

def get_dep_by_id(id):
    deps = get_file_data("database/plants.json")
    for dep in deps:
        if deps["id"] == id:
            print(dep["name"])
            print(dep["address"])